...
def recommend():
    ...
    try:
        ...
        kategori_filter = data.get('kategori', 'all')
        ...
        recommendations = model.recommend(user_nutrients, k=k, distance_metric=distance_metric, category_filter=kategori_filter)
        ...
        if nutrition_df is None:
            try:
                ...
                for col in ['energi_kal', 'protein_g', 'lemak_g', 'karbohidrat_g']:
                    if col in nutrition_df.columns:
                        nutrition_df[col] = pd.to_numeric(nutrition_df[col], errors='coerce')
            except Exception as e:
                print(f"[WARN] Could not load nutrition_df for enhancement: {e}")
                
        for rec in recommendations:
            rec['energi_kal'] = 0
            rec['kode'] = ""
            if nutrition_df is not None:
                match = nutrition_df[nutrition_df['nama_bahan'] == rec['name']]
                if not match.empty:
                    rec['energi_kal'] = float(match.iloc[0]['energi_kal']) if pd.notna(match.iloc[0]['energi_kal']) else 0
                    rec['kode'] = str(match.iloc[0]['kode'])
                    rec['nama_bahan'] = rec['name']
                    if 'nutrients' in rec:
                        rec['protein_g'] = rec['nutrients'].get('protein_g', 0)
                        rec['lemak_g'] = rec['nutrients'].get('lemak_g', 0)
                        rec['karbohidrat_g'] = rec['nutrients'].get('karbohidrat_g', 0)
            
            # Map similarity_score untuk frontend
            rec['similarity'] = rec.get('similarity_score', 0)
...
def recommend_from_profile():
    ...
    try:
        ...
        for rec in recommendations:
            ...
            # Tambahkan energi_kal dan kode dari CSV
            rec['energi_kal'] = 0
            rec['kode'] = ""
            if nutrition_df is not None:
                match = nutrition_df[nutrition_df['nama_bahan'] == rec['name']]
                if not match.empty:
                    rec['energi_kal'] = float(match.iloc[0]['energi_kal']) if pd.notna(match.iloc[0]['energi_kal']) else 0
                    rec['kode'] = str(match.iloc[0]['kode'])
                    # Pastikan field lain jika dibutuhkan frontend
                    rec['nama_bahan'] = rec['name']
                    if 'nutrients' in rec:
                        rec['protein_g'] = rec['nutrients'].get('protein_g', 0)
                        rec['lemak_g'] = rec['nutrients'].get('lemak_g', 0)
            ...
            # Map similarity_score untuk frontend hasil-rekomendasi.js
            rec['similarity'] = rec.get('similarity_score', 0)
